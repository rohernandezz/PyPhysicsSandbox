def makeRender(variations,the_color):
    ...
    #----------------------
    canvas.gravity(3,-400)  ###👈🏼👈🏼GRAVITY:(x,y)
    ...
    #if the_color == "Grey":
    background.color = Color(218,219,238,255)
    ...
    ### Physicsiphy world:
    wall_offset = 0
    #left_wall  = static_box((0-wall_offset-wall_w,0), wall_w, rh*1.5)
    #right_wall = static_box((rw+wall_offset,0), wall_w, rh*1.4)
    #left_wall.color  = wall_color
    #right_wall.color = wall_color

    #### Floor or ceiling: (both use floorH:int)
    if y_limit == "floor":
        floor      = static_box((-rw*3, rh*3), rw*7, floor_h)    
    elif y_limit == "ceiling":
        ceiling    = static_box((-rw*3, 0-floor_h), rw*7, floor_h)
    ...
    text_box_A = (200, 1000, rw*.80, 1700)
...
for wght_value in wght_list:
        for wdth_value in wdth_list:
            for slnt_value in slnt_list:
                ...
                title = f"NO-W-Grey_wdth{wdth_value}_wght{wght_value}_slnt{slnt_value}"
                print(f"💕 {title}")
